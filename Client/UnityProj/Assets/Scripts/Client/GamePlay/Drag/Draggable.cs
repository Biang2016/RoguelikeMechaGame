...
namespace Client
{
    ...
    public class Draggable : MonoBehaviour
    {
            ...
            if (_isOnDrag)
            {
                ...
                switch (dragFrom)
                {
                    case DragAreaTypes.MechaEditorArea:
                    {
                        if (isBegin)
                        {
                            dragBeginPosition_WorldObject = GetMouseAsWorldPoint() + mOffset + new Vector3(0.5f, 0, 0.5f) * ConfigManager.GridSize;
        ...
        public Vector3 GetMouseAsWorldPoint()
        {
            ...
            mousePoint.z = CameraManager.Instance.MainCamera.WorldToScreenPoint(gameObject.transform.position).z;
            return CameraManager.Instance.MainCamera.ScreenToWorldPoint(mousePoint);
        }
        ...
        public DragAreaTypes CheckMoveToArea()
        {
            ...
            Physics.Raycast(ray, out RaycastHit raycast, 1000f, LayerManager.Instance.LayerMask_DragAreas);
            if (raycast.collider)
            {

...
namespace Client
{
    public class DebugPanel : BaseUIPanel
    {
        ...
        public void AddEnemy()
        {
            ...
            MechaComponentInfo core = new MechaComponentInfo(ConfigManager.Instance.GetMechaComponentConfig("MechaComponent_BasicCore"), Quality.Common);
            ...
            for (int i = -2; i <= 2; i++)
            {
                for (int j = -4; j <= 4; j++)
                {
                    if (i != 0 && j != 0)
                    {
                        ...
                        mci = new MechaComponentInfo(ConfigManager.Instance.GetMechaComponentConfig("MechaComponent_BasicBlock"), Quality.Common);
